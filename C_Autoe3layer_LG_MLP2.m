...
w1=unifrnd(lowerb,upperb,[l1_neurons n3_neurons]);%l1*n3
net1=zeros(l1_neurons,1);%l1*1
o1=zeros(l1_neurons,1);%l1*1

w2=unifrnd(lowerb,upperb,[l2_neurons l1_neurons]);%l2*l1
net2=zeros(l2_neurons,1);%l2*1
o2=zeros(l2_neurons,1);%l2*1
...
for i=1:max_epoch_p
  for j=1:num_of_train-2
      ...
      % Back Propagation
      f_driviate_e1= diag(diag(h1*(1-h1)')); %n1*n1
      f_driviate_e2 = diag(diag(h2*(1-h2)')); %n2*n2
      f_driviate_e3 = diag(diag(h3*(1-h3)')); %n3*n3
      f_driviate= diag(diag(o1*(1-o1)')); %l1*l1
      
      w_e1=w_e1-eta_e1*(e*-1*1*(w2*f_driviate*w1*f_driviate_e3*w_e3*f_driviate_e2*w_e2*f_driviate_e1)'*input);% n1*n0 = n1*n0 - 1*l1 * l1*l1 * l1*n3 * n3*n3* n3*n2* n2*n2*  n2*n1*  n1*n1)' * 1*n0
       w_e2=w_e2-eta_e2*(e*-1*1*(w2*f_driviate*w1*f_driviate_e3*w_e3*f_driviate_e2)'*h1');% n2*n1 = n2*n1 - 1*l1 * l1*l1 * l1*n3 * n3*n3* n3*n2* n2*n2)' * 1*n1
       w_e3=w_e3-eta_e3*(e*-1*1*(w2*f_driviate*w1*f_driviate_e3)'*h2');% n3*n2 = n3*n2 - 1*l1 * l1*l1 * l1*n3 * n3*n3)' * 1*n2
       w1=w1-eta_p*e*-1*1*(w2*f_driviate)'*h3' ;  % l1*n3 = l1*n3 - 1*1 * 1*1 * (1*l1 * l1*l1)' * 1*n3
       w2=w2-eta_p*e*-1*1*o1';  % 1*l1 = 1*l1 - 1*1 * 1*1 * 1*l1
  end
  
  % confusion Train Error
